...
export const TOPUP_PRESETS = [5, 20, 50];
...
export async function promptTopupAmount(minTopup) {
  ...
  console.error("");
  console.error(`Choose top-up amount (minimum ${minTopup} USDT):`);
  presets.forEach((v, i) => {
  ...
  try {
    while (true) {
      ...
      if (Number.isInteger(n) && n === customIdx) {
        ...
        if (cn < minTopup) {
          console.error(`Amount must be at least ${minTopup} USDT.`);
          continue;
        }
        return custom;
      }
...
export async function approveFacilitator(privateKey) {
  ...
  console.error(`Pre-authorizing facilitator from ${account.address}...`);
  ...
  console.error(`Approve tx submitted: ${txHash}`);
  const receipt = await publicClient.waitForTransactionReceipt({
    hash: txHash,
    timeout: 60_000,
  });
  ...
  console.error("Approve confirmed.");
  return txHash;
}
