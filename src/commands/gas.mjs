/**
 * gas: transfer BNB from main wallet to local wallet via WalletConnect (for withdraw gas)
 */
...
export async function gas(opts) {
  ...
  if (!config.privateKey || !config.address) {
    console.error(JSON.stringify({
      error: "No local wallet found. Run 'agentos setup --check' first to auto-create one.",
    }));
    process.exit(1);
  }
